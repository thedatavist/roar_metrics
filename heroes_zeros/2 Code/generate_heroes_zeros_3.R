...
relevant_player_stats <- player_stats %>%
  mutate(
    ...
    match_year_round = as.numeric(paste0(match_year, str_pad(round.roundNumber,2,"0"))),
...
swarm_plot <- ggplot(beeswarm_player_stats,
  ...
  geom_quasirandom(aes(fill = rating_group, alpha = 0.6),
                   method = "quasirandom", width = 0.3, size = 4, stroke = 0.8, shape = 21) +
  ...
  labs(
    ...
    subtitle = "Beeswarm of AFL player ratings (≥25% TOG), highlighting top 5 & bottom 5",
    x        = "Player Rating Points",
    caption  = "Visual: Darragh Murray | Data: fitzRoy"
  ) +
  theme_minimal(base_family = "roboto", base_size = 12) +
  theme(
    plot.title            = element_markdown(size = 18, family = "bebas neue", margin = margin(b = 10), lineheight = 1),
    plot.subtitle         = element_markdown(size = 8, family = "roboto", margin = margin(b = 16)),
    axis.title.x.top      = element_text(size = 8, margin = margin(t=10, b = 4)),
...
final_plot <- swarm_plot +
  geom_text(
    ...
    aes(x = ratingPoints, y = y_position*0.5, label = label_text, colour = labelled_players$segment_colour),
    size        = 2.5,
    lineheight  = 1.5,
    family      = "roboto",
    inherit.aes = FALSE
  ...
  geom_curve(
    data       = labelled_players,
    aes(x = ratingPoints, y = y_position*0.5 - sign(y_position)*0.03,
